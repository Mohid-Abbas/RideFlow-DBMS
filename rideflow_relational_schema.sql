...
CREATE TABLE drivers (
  ...
  avail_status ENUM('ONLINE', 'OFFLINE', 'ON_TRIP') NOT NULL DEFAULT 'OFFLINE',
...
CREATE TABLE rides (
  ...
  INDEX idx_rides_pickup (pickup_loc_id),
  INDEX idx_rides_dropoff (dropoff_loc_id),
  INDEX idx_rides_fare_rule (fare_rule_id),
  ...
  -- Valid CHECK constraints (allowed)
...
CREATE TABLE payments (
  ...
  INDEX idx_payments_promo (promo_id),
...
CREATE TABLE ratings (
  ...
  INDEX idx_ratings_rated_user (rated_user),
  INDEX idx_ratings_rated_by (rated_by),
...
CREATE TABLE complaints (
  ...
  comp_status ENUM('OPEN', 'IN_PROGRESS', 'RESOLVED', 'REJECTED') NOT NULL DEFAULT 'OPEN',
  filed_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_complaints_ride (ride_id),
  INDEX idx_complaints_filed_by (filed_by),
  INDEX idx_complaints_against_user (against_user),
...
CREATE TABLE driver_earnings (
  ...
  payout_status ENUM('PENDING', 'PAID', 'HELD') NOT NULL DEFAULT 'PENDING',
...
-- Notes:
-- 1) The PDF models riders through USERS.role = 'RIDER'; there is no separate RIDERS table.
-- 2) MySQL CHECK constraints are supported in MySQL 8.0+.
-- 3) Cross-table business rules such as "rider_id must point to a user whose role is RIDER"
--    are not enforceable with FK/CHECK alone and usually need triggers or application logic.
